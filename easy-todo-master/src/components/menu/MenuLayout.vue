<template>
  <div class="menu-layout">
    <div class="input-wrapper">
      ...
      <!-- 这里的searchText指的是输入搜索文本框的文本 有文本才展示关闭按钮 -->
    ...
    <div class="todo-card-layout" @contextmenu.prevent="onMouseRightClick($event)">
      ...
      <card-item class="todo-item item-left" :card-type="1" :is-selected="isCardSelected(1)"
        :count="currentTypeStore.countInfo.todayCount" @click="onClickCard(1)" />
      <!-- 计划 -->
      <card-item class="todo-item" :card-type="2" :is-selected="isCardSelected(2)"
        :count="currentTypeStore.countInfo.planCount" @click="onClickCard(2)" />
    </div>
    <!-- 全部 -->
    <card-item class="all-card" :card-type="3" :is-selected="isCardSelected(3)"
      :count="currentTypeStore.countInfo.allCount" @click="onClickCard(3)"
      @contextmenu.prevent="onMouseRightClick($event)" />
    <type-list-layout ref="typeList" style="margin-top: 16px; margin-bottom: 30px;"
      @right-click-item="onTypeListContextSelected" />
    <div class="bottom-add bottom-divider">
      <div class="add"></div>
      <span @click="addType">添加列表</span>
    </div>
    <el-dialog v-model="showTypeDialog" :close-on-click-modal="false" width="480px" :show-close="false" align-center>
...
watch(searchText, () => {
  ...
  timer = setTimeout(() => {
    if (searchText.value) {
      ...
      // 退出的时候也就是没有searchText的时候选中all或者说上一个type
